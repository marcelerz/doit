...
/**
 * Hook for managing state that persists to storage with additional
 * data cleaning on load (e.g., filtering deleted items).
 *
 * @param storageKey - Key to use for storage
 * @param defaultValue - Default value if nothing is stored
 * @param options - Configuration including clean function
 *
 * @example
 * const [todos, setTodos, isLoaded] = usePersistedStateWithCleanup<Todo[]>(
 *   STORAGE_KEYS.TODOS,
 *   [],
 *   {
 *     migrate: migrateTodos,
 *     clean: (todos) => todos.filter(t => t.state !== "deleted"),
 *   }
 * );
 */
export function usePersistedStateWithCleanup<T>(
  storageKey: string,
  defaultValue: T,
  options: PersistedStateOptions<T> & {
    /** Function to clean data (e.g., filter deleted items) */
    clean: (data: T) => T;
  }
): PersistedStateResult<T> {
  const { migrate, clean, onLoad, skipInitialSave = true } = options;
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const loadedData = await loadFromStorage<T>(storageKey, defaultValue);
      const migratedData = migrate ? migrate(loadedData) : loadedData;
      const cleanedData = clean(migratedData);
      ...
      // Store as last saved to avoid re-saving on initial load
      if (skipInitialSave) {
        lastSavedState.current = cleanedData;
      }
