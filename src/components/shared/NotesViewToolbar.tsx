...
export interface NotesViewToolbarProps {
  // Filters state
  ...
  setSortField: (field: NotesSortField) => void;
  sortDirection: SortDirection;
  setSortDirection: (direction: SortDirection) => void;
  groupBy: NotesGroupBy;
  setGroupBy: (group: NotesGroupBy) => void;

  // View presets
  viewPresets: NotesViewPreset[];
  activePreset: string;
  onLoadPreset: (preset: NotesViewPreset) => void;
    ...
    <>
      ...
      <div className="flex items-center gap-2 lg:gap-3">
        ...
        <div
          ...
          data-tutorial="notes-search-bar"
        >
          <input
            ref={searchInputRef}
        ...
        <button
          ...
          data-tutorial="notes-filter-button"
        >
        ...
        <div className="hidden sm:flex items-center gap-2 flex-shrink-0" data-tutorial="notes-group-sort">
          <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300 whitespace-nowrap hidden lg:inline">
            Group:
          </label>
          <select
            value={groupBy}
            onChange={(e) => setGroupBy(e.target.value as NotesGroupBy)}
        ...
        <div className="hidden sm:flex items-center gap-1 flex-shrink-0">
          ...
          <select
            value={sortField}
            onChange={(e) => setSortField(e.target.value as NotesSortField)}
            className="px-2 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
            title="Sort by"
          >
        ...
        <button
          ...
          data-tutorial="notes-save-preset"
        >
        ...
        {features?.batchProcessing && (
          <button
            ...
            data-testid="notes-selection-mode-button"
          ...
          <div ref={exportMenuRef} className="hidden lg:block relative flex-shrink-0">
            ...
            {isExportMenuOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-zinc-800 rounded-lg shadow-lg border border-zinc-200 dark:border-zinc-700 py-1 z-50">
                <ExportMenuItems onExport={handleExport} hasActiveFilters={hasActiveFilters} noun="notes" />
              </div>
            )}
          </div>
        )}
