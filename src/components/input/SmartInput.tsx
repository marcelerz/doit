...
function SmartEditableInput({
  ...
  workHoursSettings,
  ref,
}: SmartEditableInputProps & { ref?: React.Ref<SmartEditableInputHandle> }) {
  ...
  useEffect(() => {
    if (
      ...
      const { fragment, tokens, plainText } =
        renderTokensFromText(initialValue);
  ...
  const buildTokenRegex = (): {
    ...
    for (const { type, symbol } of peopleMarkers) {
      ...
      const allNames = availablePeople.flatMap((p) => [
        p.name,
        ...p.alternatives,
      ]);
    ...
    const allProjects = availableProjects.flatMap((p) => [
      p.name,
      ...p.alternatives,
    ]);
    if (allProjects.length > 0) {
      const sortedProjects = allProjects.sort((a, b) => b.length - a.length);
      const namesPattern = sortedProjects
        .map((name) => name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))
        .join("|");
    ...
    const allPriorities = availablePriorities.flatMap((p) => [
      p.name,
      ...p.alternatives,
    ]);
    if (allPriorities.length > 0) {
      const sortedPriorities = allPriorities.sort(
        (a, b) => b.length - a.length,
      );
      const namesPattern = sortedPriorities
        .map((name) => name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))
        .join("|");
      const regex = new RegExp(`!!(${namesPattern})(?=\\s|$)`, "gi");
  ...
  const renderTokensFromText = (
    ...
    const detectedDates = detectDatesInText(
      text,
      new Date(),
      dateTimeSettings,
      workHoursSettings,
    ...
    for (const detected of detectedPeople) {
      ...
      const overlapsDate = tokens.some(
        (t) =>
          (t.type === "dueDate" || t.type === "recurring") &&
    ...
    const autoDetectedTypesToRemove = new Set([
      "dueDate",
      "duration",
      "recurring",
      "dependency",
      ...
      const getTokenColor = (tok: TokenMatch): string => {
        ...
        if (tok.isAutoDetected && tok.autoDetectedType) {
          if (tok.type === "recurring")
            return markerColors["recurring"] || "#e0f2f1";
          if (tok.type === "duration")
            return markerColors["duration"] || "#fff4e6";
          if (tok.type === "dueDate")
            return markerColors["dueDate"] || "#fce4ec";
        }
        ...
        return (
          markerColors[tok.type as keyof typeof markerColors] ||
          fallbackColors[tok.type] ||
          "#f5f5f5"
        );
      };
  ...
  const handleInput = () => {
    ...
    if (lastMarkerPos >= 0) {
      const textAfterMarker = textBeforeCaret.substring(
        lastMarkerPos + lastMarker.length,
      );
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (autocomplete.show) {
      const totalItems =
        autocomplete.options.length + (autocomplete.showAddNew ? 1 : 0);
  ...
  const insertAutocomplete = (value: string) => {
    ...
    const afterText =
      spaceAfterIndex >= 0 ? afterMarker.substring(spaceAfterIndex) : "";

    // Build the new text with the completed name
    const newText =
      beforeMarker + autocomplete.marker + finalValue + " " + afterText;
        ...
        >
          {autocomplete.options.map((option, idx) => (
            <button
              ...
              onMouseEnter={() =>
                setAutocomplete((prev) => ({ ...prev, selected: idx }))
              }
