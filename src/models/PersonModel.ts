...
import type { Person, Comment, ActivityEntry } from "@/types/settings";
...
export class PersonModel {
  /**
   * The raw Person object wrapped by this model.
   * Access this when you need to save or pass the data to storage/hooks.
   */
  public readonly raw: Person;
  ...
  /**
   * Whether this person is currently active (not archived)
   */
  ...
  /**
   * Whether this person is archived
   */
  ...
  /**
   * Check if this person can be archived
   * @returns Validation result with reason if not allowed
   */
  canArchive(): { canArchive: boolean; reason?: string } {
    if (this.archived) {
      return { canArchive: false, reason: "Person is already archived" };
    }
    return { canArchive: true };
  }

  /**
   * Check if this person can be unarchived
   * @returns Validation result with reason if not allowed
   */
  canUnarchive(): { canUnarchive: boolean; reason?: string } {
    if (!this.archived) {
      return { canUnarchive: false, reason: "Person is not archived" };
    }
  ...
  /**
   * Whether this person has any comments
   */
  ...
  /**
   * Whether this person has any activity
   */
   ...
   * @returns "John Doe (Johnny, JD)" or just "John Doe"
   */
  ...
  /**
   * Get a summary of metadata for this person
   * @returns Formatted string like "3 todos • 2 comments • Active"
   */
   ...
   * @returns First letters of first and last name, or first 2 letters
   */
  get initials(): string {
  ...
  /**
   * Check if this person matches any of the given names or alternatives
   * Used for parsing @mentions in todo text
   */
  matchesAnyName(names: string[]): boolean {
    const lowerNames = names.map((n) => n.toLowerCase());
    const personNames = [this.name, ...this.alternatives].map((n) => n.toLowerCase());
    return personNames.some((name) => lowerNames.includes(name));
  }
